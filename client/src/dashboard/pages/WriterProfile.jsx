...
const BEATS = ['Campus News', 'Sports', 'Culture & Events', 'Science & Tech', 'Opinion', 'Administration'];
...
const WriterProfile = () => {
  ...
  const [avatarPreview, setAvatarPreview] = useState(
    'https://ui-avatars.com/api/?name=Sadia+Rahman&background=D97706&color=fff&size=200&bold=true'
  );
  ...
  const [formData, setFormData] = useState({
    ...
    beat: 'Campus News',
    email: 'sadia@rajshahicollege.edu.bd',
    phone: '+880 1XXX-XXXXXX',
    twitter: '@sadia_writes',
    bio: 'Covering campus events, examinations, and academic achievements for Rajshahi College News Portal.',
  });
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="h-24 sm:h-32 md:h-36 bg-gradient-to-r from-amber-800 via-amber-700 to-amber-500" />

        <div className="px-4 sm:px-6 md:px-8 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between -mt-10 sm:-mt-12 md:-mt-14 gap-4">
            <div className="flex flex-col sm:flex-row sm:items-end gap-4">
              <div className="relative w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 flex-shrink-0 mx-auto sm:mx-0">
                ...
                <button
                  ...
                  className="absolute bottom-0 right-0 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-amber-700 text-white flex items-center justify-center border-2 border-white hover:bg-amber-800 transition"
                >
                  <FiCamera size={14} />
                </button>
                <input
              ...
              <div className="pb-1 text-center sm:text-left">
                <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
                  <h1 className="text-lg sm:text-xl font-bold text-gray-900">{formData.fullName}</h1>
                ...
                <p className="text-xs text-amber-700 mt-1 font-medium">{formData.beat}</p>
              </div>
            </div>

            <div className="grid grid-cols-3 sm:flex gap-3 sm:gap-6 w-full sm:w-auto pb-1">
              <div className="text-center">
                <p className="text-base sm:text-lg font-bold text-gray-900 leading-tight">42</p>
                <p className="text-[11px] sm:text-xs text-gray-500 leading-tight">Articles Published</p>
              </div>
              <div className="text-center sm:border-l sm:border-gray-100 sm:pl-6">
                <p className="text-base sm:text-lg font-bold text-gray-900 leading-tight">18.4k</p>
                <p className="text-[11px] sm:text-xs text-gray-500 leading-tight">Total Views</p>
              </div>
              <div className="text-center sm:border-l sm:border-gray-100 sm:pl-6">
                <p className="text-base sm:text-lg font-bold text-gray-900 leading-tight">Mar 2024</p>
                <p className="text-[11px] sm:text-xs text-gray-500 leading-tight">Member Since</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'info' && (
        <form onSubmit={handleSaveInfo} className="mt-6 bg-white rounded-xl border border-gray-100 shadow-sm p-5 sm:p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Beat / Category</label>
              <div className="relative">
                <FiTag className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
                <select
                  value={formData.beat}
                  onChange={(e) => handleInfoChange('beat', e.target.value)}
                  className="w-full pl-9 pr-9 py-2.5 rounded-lg border border-gray-300 outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-100 transition appearance-none bg-white cursor-pointer"
                >
                  {BEATS.map((b) => (
            ...
            <div>
              ...
              <div className="relative">
                <FiMail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
                <input
                  type="email"
                  required
            ...
            <div>
              ...
              <div className="relative">
                <FiPhone className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
                <input
                  type="tel"
            ...
            <div>
              ...
              <div className="relative">
                <FiTwitter className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
                <input
                  type="text"
                  value={formData.twitter}
                  onChange={(e) => handleInfoChange('twitter', e.target.value)}
                  className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-300 outline-none focus:border-amber-700 focus:ring-2 focus:ring-amber-100 transition"
                />
              </div>
            </div>
