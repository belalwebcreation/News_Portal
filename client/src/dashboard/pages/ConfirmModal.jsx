...
import { AlertTriangle, ShieldAlert, Trash2, ArrowUpRight, ArrowDownRight, X } from "lucide-react";
...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message, type = "default", isLoading = false }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-150">
      <div 
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-slate-900/5 animate-in zoom-in-95 duration-150"
      >
        ...
        <div className="mt-4 text-sm text-slate-600 leading-relaxed bg-slate-50/80 rounded-xl p-3.5 border border-slate-100">
          {message}
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          <button
            ...
            onClick={onClose}
            disabled={isLoading}
            className="rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
